...
trait ScalaLift extends ScalaDefsExps {
  ...
  implicit class NumericOps[T:Numeric:Manifest](x: Exp[T]) {
    def *(y: Exp[T]): Exp[T] = BinaryOp[T,T](x, "*", y)
    def +(y: Exp[T]): Exp[T] = BinaryOp[T,T](x, "+", y)
    def %(y: Exp[T]): Exp[T] = BinaryOp[T,T](x, "%", y)
    def toDouble: Exp[Double] = Method[T,Double](x, "toDouble")
  }

  implicit def liftRangeOps(x: Int): RangeOps = new RangeOps(liftInt(x))
  implicit class RangeOps(x: Exp[Int]) {
    def until(y: Exp[Int]) = RangeExp(x,y)
  }
