package org.scala_lang.virtualized.tutorial.prob
...
trait ProbLang extends /*EmbeddedControls with*/ ProbIntf {
  ...
  implicit class RandBoolean(x: Rand[Boolean]) {
    def &&(y: Rand[Boolean]): Rand[Boolean] = liftOp2(x,y)(_ && _) // short circuit ??
  }
  implicit class RandA[A](x: Rand[A]) {
    def ===(y: Rand[A]): Rand[Boolean] = liftOp2(x,y)(_ == _)
  }
  implicit class RandInt(x: Rand[Int]) {
    def +(y: Rand[Int]): Rand[Int] = liftOp2(x,y)(_ + _)
  }
}
