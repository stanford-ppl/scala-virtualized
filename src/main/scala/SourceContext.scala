...
object SourceContext {
  ...
  def apply(path: String, fileName: String, line: Int, column: Int, methodName: String): SourceContext = 
    new ConcreteSourceContext(path, fileName, line, column, methodName)

  private class ConcreteSourceContext(val path: String, val fileName: String, val line: Int, val column: Int, val methodName: String) extends SourceContext
}
...
private object SourceContextMacro {
  def impl(c: Context): c.Expr[SourceContext] = {
    ...
    c.Expr(q"SourceContext($path, $filename, $line, $column, $methodName)")
  }
}
